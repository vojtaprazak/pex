...
if [int(dgt) for dgt in numpy.__version__.split('.')[:2]] < [1, 4]:
    sys.stderr.write('TEMPy requires numpy v1.4 or later, you can find it at: '
                     'http://numpy.scipy.org\n')
    sys.exit()
...
if [int(dgt) for dgt in Bio.__version__.split('.')[:2]] < [1, 4]:
    sys.stderr.write('TEMPy requires Biopython v1.4 or later, you can find it at: '
                     'http://biopython.org/wiki/Main_Page\n')
    sys.exit()
