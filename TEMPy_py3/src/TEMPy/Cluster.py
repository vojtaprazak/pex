...
class Cluster:
    ...
    def RMSD_ensemble(self,rank_fit_ensemble,ensemble_list):
        """
    ...
    def rank_fit_ensemble(self,ensemble_list,score,res_target_map,sigma_coeff,number_top_mod=0,write=False,targetMap=False):
        """
        ...
        if score in ['NV','NV_Sobel','NV_Laplace']:
            list_ordered=sorted(list_to_order, key=lambda x: x[2],reverse=False)
        else:
