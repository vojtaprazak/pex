...
def pcle_links(csvfile, modfile, max_dist, apix, maxnbrs=30, outfile=''):
    dists, nbrs = pcle_dist_from_nbr(PEETMotiveList(csvfile), PEETmodel(modfile),apix,maxnbrs)
    links = []
    for n in range(len(nbrs)):
    ...
    if outfile:
        ...
        for poi in range(len(points)):
            p1 = points[poi]
            markers.append(etree.SubElement(inpfile, 'marker', id=str(poi), x=str(p1.x),\
                                         y=str(p1.y), z=str(p1.z),r=str(100./255), g=str(149./255), b=str(237./255), radius='3.0'))
            for l in links[poi]:
                marker_links.append(etree.SubElement(inpfile, 'link', radius='3.0', id1=str(poi), id2=str(l),r=str(100./255), g=str(149./255), b=str(237./255)))
        if outfile[-4:] != '.cmm':
            outfile += '.cmm'
        out = open(outfile, 'w')
        doc.write(out, pretty_print=True)
...
def compare_pcle_dist(csv1, mod1, csv2, mod2):
    points1 = csv1.get_all_offsets()+mod1.get_all_points()
    points2 = csv2.get_all_offsets()+mod2.get_all_points()
    dists = ((points1-points2)**2).sum(axis=1)**0.5
    return dists

def compare_pcle_angles(csv1, csv2):
    mat1 = csv1.angles_to_rot_matrix()
    mat2 = csv2.angles_to_rot_matrix()
    scores = array([linalg.norm(identity(3)-matrix(mat1[x])*matrix(mat2[x]).T) for x in range(len(mat1))])
    # Empirically determined approx. conversion to degrees
    degs = 114.53812076116014*np.arcsin(scores/2.828319426908065)
    return scores, degs

def compare_cc(csv1, csv2):
    return array(csv2.mlist)[:,0]- array(csv1.mlist)[:,0]

def compare_refs(peetdir, base_name, num_iter):
    from ScoringFunctions import ScoringFunctions
    from MapParser_f32_new import MapParser
    sf = ScoringFunctions()
    ccs = []
    for x in range(1, num_iter-1):
        a = MapParser.readMRC(peetdir+'/'+base_name+'Ref'+str(x)+'.mrc')
        b = MapParser.readMRC(peetdir+'/'+base_name+'Ref'+str(x+1)+'.mrc')
        ccs.append(sf.CCC(a,b))
    return ccs

def check_mask_bias(peetdir, base_name, num_iter, iter_start=1):
    from MapParser_f32_new import MapParser

    ratios = []
    for x in range(iter_start, num_iter+1): 
        masked = MapParser.readMRC(peetdir+'/'+base_name+'_Ref'+str(x)+'.mrc')
        unmasked = MapParser.readMRC(peetdir+'/unMasked'+base_name+'_Ref'+str(x)+'.mrc')
        unmask_mean = unmasked.fullMap.mean()+ 0.5*unmasked.std()
        mask_pos = float(masked.fullMap[masked.fullMap > unmask_mean].sum())/(masked.fullMap > unmask_mean).sum()
        background = unmasked.fullMap-masked.fullMap
        if background.sum() != 0:
            bgd_pos = float(background[background > unmask_mean].sum())/(background > unmask_mean).sum()
            ratios.append(mask_pos/bgd_pos)
            print(mask_pos, bgd_pos, unmask_mean)
        else:
            print('No mask was used in this average!')
            return
    return ratios


def get_cc_info(csvlist):
    ccs = [array(csvlist[x].mlist)[:,0] for x in range(len(csvlist))]
    means = [mean(x) for x in ccs]
    cc_diffs = [compare_cc(csvlist[x], csvlist[x+1]) for x in range(len(csvlist)-1)]
    return ccs, means, cc_diffs

def arcsin_func(x, a, c):
    return c*np.arcsin(a*x)

def make_dummy_ang_score():
    angs = []
    score = []
    a = axis_angle_to_matrix(1,0,0,0)
    for x in range(0,180,1):
        b = axis_angle_to_matrix(1,0,0,x)
        angs.append(x)
        score.append(linalg.norm(identity(3)-a*b.T))
    return angs, score
    

def get_pcle_info(csvfile, modfile, apix, outfile):
    csv = PEET_motive_list(csvfile)
