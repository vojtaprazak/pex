...
class IMOD_comfile:
    ...
    def __init__(self, rec_dir, com_name,  make_paths_absolute=True,
                 **kwargs):
    ...
    def get_imod_base_name(self, return_keys =  False):
        ...
        if prefix == '':
            print('Warning: Incosistent naming in com script! Assuming files are named something.someextension.')
        #This is likely not fully general... but it takes care of the 
        #case where all extensions are .something
        if prefix.endswith('.'):
            prefix = prefix[:-1]
        if prefix == '':
            extensions = ['.' + x.split('.')[-1] for x in all_paths]
        else:
    ...
    def remove(self, key):
        if key in self.dict:
            del(self.dict[key])
        if key in self.separator_dict:
            del(self.separator_dict[key]     )      
        if key in self.b_dict:
            del(self.b_dict[key])
        if key in self.b_separator_dict:
            del(self.b_separator_dict[key])
    ...
    def write_comfile(self, out_dir = False, change_name = False):
        ...
        def guess_separator(key):
            ...
            if key == 'EXCLUDELIST':
                #@testing
                print(key, str(self.separator_dict[key]), str(self.dict[key]))
        ...
        with open(out_file, 'w') as f:
            ...
            for key in self.dict.keys():
                ...
                try:
                    ...
                    print('type key:value', type(key), type(self.dict[key]))
                    raise
    ...
    def get_command_list(self, exclude_keys = ['RADIAL', 'FalloffIsTrueSigma',
        ...
        def guess_separator(key):
            #if key in self.separator_dict.keys(): #in the com e.g. fullimage is space separated, cmd needs comma
            #    return self.separator_dict[key]
