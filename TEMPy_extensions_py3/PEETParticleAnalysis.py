...
def csvfile_to_chim_markers(csvfile, modfile, outfile, d=30, classID=-1, dummy=[0,1,0], head_rad=8.0, cc_range=[-2,-2]):
    ...
    out.write(etree.tostring(doc, pretty_print=True, encoding='unicode'))
    out.close()
...
def pcle_dist_from_nbr(csvfile, modfile, apix, noOfNbrs=1, outfile=''):
    ...
    # Remove particle itself from each neighbour/distance list
    same_check = array([arange(len(dists))]).T
    same_check = repeat(same_check, noOfNbrs+1, axis=1)
    same_check = nbrs != same_check
    nbrs = nbrs[same_check].reshape((len(nbrs), noOfNbrs))
    dists = dists[same_check].reshape((len(nbrs), noOfNbrs))
    
    if outfile:
        savetxt(outfile+'.txt', dists*apix)
        savetxt(outfile+'_flat.txt', (dists*apix).flatten())
    return dists*apix, nbrs
...
def pcle_links(csvfile, modfile, max_dist, apix, maxnbrs=6, outfile='', head_radius = '8.0'):
    ...
    if outfile:
        ...
        for poi in range(len(points)):
            p1 = points[poi]
            markers.append(etree.SubElement(inpfile, 'marker', id=str(poi), x=str(p1[0]),\
                                         y=str(p1[1]), z=str(p1[2]),r='1', g='1', b='1', radius=head_radius))
            for l in links[poi]:
