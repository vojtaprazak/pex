...
class PEETPRMFile:
    ...
    def make_absolute_paths(self):
        if type(self.prm_dict['reference']) == str:
            self.prm_dict['reference'] = os.path.realpath(self.prm_dict['reference'])
        if self.prm_dict['maskType'] != 'sphere' and self.prm_dict['maskType'] != 'cylinder':
            self.prm_dict['maskType'] = os.path.real_path(self.prm_dict['maskType'])
                                                 
        new_fnvol = []
        for x in self.prm_dict['fnVolume']:
            new_fnvol.append(os.path.realpath(x))
        self.prm_dict['fnVolume'] = new_fnvol
        
        new_mod = []
        new_motl = []
        for p in range(len(self.prm_dict['fnModParticle'])):
            new_mod.append(os.path.realpath(self.prm_dict['fnModParticle'][p]))
            new_motl.append(os.path.realpath(self.prm_dict['initMOTL'][p]))
        self.prm_dict['fnModParticle'] = new_mod
        self.prm_dict['initMOTL'] = new_motl
    ...
    def clean_pcles(self, max_dist, ite, outfile_dir, clean_ccc=False, cccmin=1., cccmax=100., stdev_units=True, \
                              no_of_nbrs=30, offset_mv=True, writeprm=False, verbose=True, intraclass_rm=True):
        motls = self.get_MOTLs_from_ite(ite)
        models = self.prm_dict['fnModParticle']
        new_motls = []
        new_models = []
        ...
        for x in range(len(models)):
            ...
            m1,m2, bef, aft = remove_duplicates(os.path.abspath(motls[x]), os.path.abspath(models[x]), max_dist, \
                                                ...
                                                no_of_nbrs=no_of_nbrs, offset_mv=offset_mv, verbose=verbose, intraclass_rm=intraclass_rm)
            before += bef
            after += aft
            if aft != 0:
                new_motls.append(os.path.abspath(outbase+'.csv'))
    ...
    def remove_bad_symm_pcles(self, ite, outfile_dir, t_tol, r_tol, sym,\
        ...
        for x in range(len(models)):
            if verbose:
                print("Removing badly symmetrised particles from "+os.path.basename(motls[x]))
            outbase = outfile_dir+'/'+os.path.basename(motls[x])[:-4]+'_symmclean'
            new_csv, new_mod, pcle_types, bef, aft = check_symm_pcles(os.path.abspath(motls[x]), os.path.abspath(models[x]), sym,\
                                                            t_tol=t_tol, r_tol=r_tol, verbose=verbose, no_of_nbrs=no_of_nbrs,\
                                                            outfile=outbase, offset_mv=offset_mv)
            before += bef
            after += aft
            if aft != 0:
                new_motls.append(os.path.abspath(outbase+'.csv'))
        ...
        new_prm.prm_dict['fnVolume'] = new_toms
        new_prm.prm_dict['tiltRange'] = new_tiltrange
        new_prm.prm_dict['fnOutput'] = new_prm.prm_dict['fnOutput']+'_fromIter'+str(ite)+'_symmclean'
        if writeprm:
            new_prm.write_prm_file(outfile_dir+'/'+new_prm.prm_dict['fnOutput']+'.prm')
        return new_prm
    ...
    def remove_offedge_pcles(self, ite, dist_from_edge, outfile_dir, writeprm=True, offset_mv=True, verbose=True):
        ...
        for x in range(len(models)):
            if verbose:
                print("Removing duplicates from "+os.path.basename(motls[x]))
            if type(dist_from_edge) != list:
                outbase = outfile_dir+'/'+os.path.basename(motls[x])[:-4]+'_remedge_'+str(dist_from_edge)
            else:
                outbase = outfile_dir+'/'+os.path.basename(motls[x])[:-4]+'_remedge_%0d_%0d_%0d'%tuple(dist_from_edge)
        ...
        if after == 0:
            ...
            if type(dist_from_edge) != list:
                ...
                new_prm.prm_dict['fnOutput'] = new_prm.prm_dict['fnOutput']+'_fromIter'+str(ite)+'_remedge_%0d_%0d_%0d'%tuple(dist_from_edge)
            if writeprm:
                new_prm.write_prm_file(outfile_dir+'/'+new_prm.prm_dict['fnOutput']+'.prm')
            return new_prm
    ...
    def clean_spheres(self, ite, outfile_dir, writeprm=False, fixed_radius=None, rad_extra=10, max_num_sph=20, verbose=True):
        motls = self.get_MOTLs_from_ite(ite)
        models = self.prm_dict['fnModParticle']
        new_motls = []
        new_models = []
        ...
        for x in range(len(models)):
            if verbose:
                print("Removing particles from "+os.path.basename(motls[x]))
            outbase = outfile_dir+'/'+os.path.basename(motls[x])[:-4]+'_cln_sph'
            new_motl, new_mod, in_num, out_num = clean_spheres(os.path.abspath(motls[x]), os.path.abspath(models[x]), fixed_rad=fixed_radius,
                                                              rad_extra=rad_extra, max_num_sph=20, outfiletemp=outbase, verbose=verbose)
            before += in_num+out_num
            after += in_num
            if in_num != 0:
                new_motls.append(os.path.abspath(outbase+'.csv'))
                new_models.append(os.path.abspath(outbase+'.mod'))
        ...
        new_prm.prm_dict['fnVolume'] = new_toms
        new_prm.prm_dict['tiltRange'] = new_tiltrange
        new_prm.prm_dict['fnOutput'] = new_prm.prm_dict['fnOutput']+'_fromIter'+str(ite)+'_spherecleaned'
        if writeprm:
            new_prm.write_prm_file(outfile_dir+'/'+new_prm.prm_dict['fnOutput']+'.prm')
        return new_prm
    ...
    def bin_all(self, bin_val, ite, outfile_dir, writeprm=True, offset_mv=True):
        motls = self.get_MOTLs_from_ite(ite)
        models = self.prm_dict['fnModParticle']
        new_motls = []
        new_models = []
    ...
    def get_tetra(self, ite, diameter, outfile_dir, orient=[1,0,0,60], writeprm=True):
        motls = self.get_MOTLs_from_ite(ite)
        models = self.prm_dict['fnModParticle']
        new_motls = []
        new_models = []
        ...
        for x in range(len(models)):
            csv = PEETMotiveList(os.path.abspath(motls[x]))
            mod = PEETmodel(os.path.abspath(models[x]))
            outbase = os.path.abspath(outfile_dir+'/'+os.path.basename(motls[x])[:-4]+'_tetra')
            new_csv, new_mod = get_tetra_from_run(csv, mod, diameter, outfile=outbase, orient=orient)
            new_motls.append(outbase+'.csv')
            new_models.append(outbase+'.mod')
        new_prm = self.deepcopy()
        new_prm.prm_dict['fnModParticle'] = new_models
        new_prm.prm_dict['initMOTL'] = new_motls
        new_prm.prm_dict['fnVolume'] = [os.path.abspath(x) for x in self.prm_dict['fnVolume']]
        new_prm.prm_dict['fnOutput'] = new_prm.prm_dict['fnOutput']+'_fromIter'+str(ite)+'_tetra'
        if writeprm:
            new_prm.write_prm_file(outfile_dir+'/'+new_prm.prm_dict['fnOutput']+'.prm')
        return new_prm
    ...
    def get_hexons(self, ite, diameter, tri_num, outfile_dir, orient='i2', writeprm=True, justedges=False):
        motls = self.get_MOTLs_from_ite(ite)
        models = self.prm_dict['fnModParticle']
        new_motls = []
        new_models = []
        ...
        for x in range(len(models)):
            ...
            new_csv, new_mod = get_hexons_from_run(csv, mod, diameter, tri_num, outfile=outbase, orient=orient, justedges=justedges)
            new_motls.append(outbase+'.csv')
            new_models.append(outbase+'.mod')
        new_prm = self.deepcopy()
        new_prm.prm_dict['fnModParticle'] = new_models
