...
from numpy import  array, int16, int32, float32, float64, zeros, real, argwhere, diag, histogram, dot, matrix, amin, arange, indices, ravel, all as all_points, delete, transpose, ma, sum as numsum,median,sqrt as srt, digitize
...
class Map:
    ...
    def __init__(self, fullMap, origin, apix, filename, header=[], ext_header=[]):
        """
    ...
    def copy(self):
        ...
        """
        copy = Map(self.fullMap.copy(), self.origin[:], self.apix, self.filename, self.header[:], self.ext_header[:])
        return copy
    ...
    def rotate_by_axis_angle(self, x, y, z, angle, CoM, rad=False, cval='min'):
        """
        ...
        offset = CoM-newCoM
        # Apply transform
        newMap = self.matrix_transform(m, offset.x, offset.y, offset.z, cval=cval)
        return newMap

    def rotate_by_euler(self, x, y, z, CoM, rad=False, cval='min'):
        """
    ...
    def rotate_by_matrix(self, mat, CoM, rad=False, cval='min', order=3):
        """
        ...
        offset = CoM-newCoM
        # Apply transform
        newMap = self.matrix_transform(mat, offset.x, offset.y, offset.z, cval=cval, order=3)
        return newMap
    ...
    def matrix_transform(self, mat, x=0, y=0, z=0, cval='min', order=3):
        """
        ...
        newMap.fullMap = newMap.fullMap.swapaxes(0,2)
        if cval == 'min':
            newMap.fullMap = affine_transform(newMap.fullMap, mat, offset=(off_x, off_y, off_z), cval=self.min(), order=order)
        else:
            newMap.fullMap = affine_transform(newMap.fullMap, mat, offset=(off_x, off_y, off_z), cval=cval, order=order)
        newMap.fullMap = newMap.fullMap.swapaxes(0,2)
        return newMap
...
##   that should be used in the blured but must be checked     
    ...
    def std(self):
        ...
        m = self.mean()
        sum_of_sq = 0
        for z in self.fullMap:
            sum_of_sq += ((z-m)**2).sum()
        #return sum_of_sq
        return (sum_of_sq/self.fullMap.size)**0.5
...
#         print low, high
    ...
    def write_to_MRC_file(self, mrcfilename):
        ...
        if len(self.header) != 0:
            h = self.create_header(True) # h = self.header_to_binary()
        else:
            h = self.create_header(True)
    ...
    def create_header(self, binarise=False):
        """
    ...
    def update_header(self):
        ...
        ispg = int32(0)
        nsymbt = int32(self.header[23])
        creatid = int16(self.header[24])
        extra1 = [int16(0)]*15
        nint = int16(self.header[40])
        nreal = int16(self.header[41])
        extra2 = [int16(0)]*10
        imodStamp = int32(self.header[52])
        imodFlags = int32(self.header[53])
        idtype = int16(self.header[54])
        lens = int16(self.header[55])
        nd1 = int16(self.header[56])
        nd2 = int16(self.header[57])
        vd1 = int16(self.header[58])
        vd2 = int16(self.header[59])
        tiltangles = self.header[60:66]
        ...
        self.header = [nx, ny, nz, mode, nxstart, nystart, nzstart, mx, my, mz, xlen, ylen, zlen, alpha, beta, gamma,\
                       mapc, mapr, maps, amin, amax, amean, ispg, nsymbt, creatid]
        self.header.extend(extra1)
        self.header.extend([nint, nreal])
        self.header.extend(extra2)
        self.header.extend([imodStamp, imodFlags, idtype, lens, nd1, nd2, vd1, vd2])
        self.header.extend(tiltangles)
        self.header.extend([xorigin, yorigin, zorigin])
        self.header.extend(mapword)
        self.header.extend([byteorder, byteorder, '\x00', '\x00'])
        self.header.extend([rms, nlabels, self.header[-1]])
    ...
    def header_to_binary(self):
        ...
        fm_string = '<'+(10*'l')+(6*'f')+(3*'l')+(3*'f')+(2*'l')+'H'+(27*'H')+(2*'l')+(6*'H')+(9*'f')+(8*'c')+'fl800s' #10l6f3l3f2lH27H2l6H9f8cfl800s'
        packed = binary.pack(fm_string, *self.header)
        return packed
