...
import numpy
from numpy import array, fromfile, flipud,isnan
...
class MapParser:
    ...
    def readMRCHeader(filename, endian = '<'):
        ...
        fm_string = endian+(10*'l')+(6*'f')+(3*'l')+(3*'f')+(2*'l')+'H'+(27*'H')+(2*'l')+(6*'H')+(9*'f')+(8*'c')+'fl'
    ...
    @staticmethod
    def readMRC(filename, final_dtype='float32', chunk=[]):
        """
        ...
        if not chunk:
            ...
            if any(array([z1,y1,x1]) < 0) or any(array([z2,y2,x2]) > box_size):
        ...
        map_data=array(map_data, dtype=final_dtype)
        ...
        f.close()
        return Map(map_data, origin, apix, filename, header=header, ext_header=ext_head)

    #BROKEN
    @staticmethod
    def readXPLOR(filename, user_origin=None, user_box_size=None):
        """
