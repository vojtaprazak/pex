...
def clean_using_marker_file(csvfile, modfile, markerfile, outfile='', verbose='True', marker_out=False, by_colour=True):
    csv = PEETMotiveList(csvfile)
    mod = PEETmodel(modfile)
    ...
    for m in range(len(mod)):
        if by_colour:
            if len(csv) == len(marker_ids)/2:
                raise TypeError("Motive list and marker file have lengths of "+str(len(csv))+" and "+str(len(marker_ids))+". Lengths must be the same! Aborting...")
