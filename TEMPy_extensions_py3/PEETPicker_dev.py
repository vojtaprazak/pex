...
# coords from https://en.wikipedia.org/wiki/Tetrahedron#Formulas_for_a_regular_tetrahedron
def get_tetrahedral(radius, outfile=False, orient=False):
    x = 1./(2**0.5)
    vert = []
    vert.append(Vector(1.,0,-x))
    vert.append(Vector(-1.,0,-x))
    vert.append(Vector(0,-1.,x))
    vert.append(Vector(0,1.,x))

    vert = [v.unit()*radius for v in vert]

    if orient:
        mat = axis_angle_to_matrix(*orient)
        vert = [v.matrix_transform(mat) for v in vert]

    fullcon =[]
    fullcon.append([1,2,3])
    fullcon.append([0,2,3])
    fullcon.append([0,1,3])
    fullcon.append([0,1,2])
    ...
    if outfile:
        ...
        for x in range(len(vert)):
                ...
                for y in range(x, len(vert)):
                    s.footer += "%s" %(str(y+1).rjust(5))
                s.footer += '\n'
        s.write_to_PDB(outfile)
    return vert, fullcon

def get_tetra_from_run(csv, mod, radius, outfile='', orient=[1,0,0,60]):
    tetra, fullcon = get_tetrahedral(radius, orient=orient)
    ...
    for p in range(len(mats)):
        new_points = [point.matrix_transform(mats[p])+mod_with_off.get_vector(p) for point in tetra]
        for v in range(len(new_points)):
            ...
            r = fullcon[v][random.randint(0,2)]
            new_xangle = angle_for_x_axis_to_point(new_points[v], new_points[r], newmat)
...
def get_pentons(diameter, outfile=False, orient='i2'):
    ...
    for v in range(len(vert)):
        ...
        if orient == 'i1':
            ...
            mat = euler_matrix(radians(-90),radians(31.7175),0,'rzyz')[:3,:3]
            vert[v] = vert[v].matrix_transform(mat)
        else:
...
def get_hexons(radius, tri_num=4, outfile='', orient='i2', curved=True, justedges=False, user_pents=[], return_cons=False):
    pentons, cons, simplices, fullcon = get_pentons(radius, orient=orient)
    if len(user_pents) != 0:
        pentons = user_pents
        kdtree = KDTree(pentons)
        dists, nbrs = kdtree.query(pentons, 6)
        cons = []
        for n in range(len(nbrs)):
            this_nbrs = nbrs[n][1:][nbrs[n][1:] > n-1]
            cons.append(this_nbrs)
        
        simplices = ConvexHull(pentons).simplices
        pentons = [Vector(*p) for p in user_pents]
    ...
    for p in range(len(pentons)):
        ...
        for c in range(len(cons[p])):
            ...
            if curved:
                ...
                hexons.extend(split_line(v1,v2,tri_num))
    ...
    if not justedges:
        for s in range(len(simplices)):
            ...
            if curved:
                ...
                h1 = split_line(v1,v2,tri_num)
                h2 = split_line(v1,v3,tri_num)

            for h in range(1, len(h1)):
                if curved:
                    hexons.extend(split_arc(h1[h],h2[h],h))
                else:
                    hexons.extend(split_line(h1[h],h2[h],h))
    
    if outfile:
        s = vectorlist_to_pdb(hexons)
        s.write_to_PDB(outfile)
    if return_cons:
        return hexons, pentons, cons
    else:
        return hexons, pentons, cons


def get_icos_symm_pos_from_vector(vector, orient='i2', outfile=None):
    new_vecs = [vector.to_array()]
    diameter = vector.mod()
    vert, con, simplices, fullcon = get_pentons(diameter, orient=orient)
    dists = [(d-vector).mod() for d in vert]
    p_ind = np.where(dists==min(dists))[0][0]
    near_p = vert[p_ind]
    p_dist = [(d-near_p).mod() for d in vert]
    max_p_ind = np.where(p_dist == max(p_dist))[0][0]
    for x in range(1,5):
        rot_mat = axis_angle_to_matrix(near_p.x, near_p.y, near_p.z, x*72)
        new_vecs.append(vector.matrix_transform(rot_mat).to_array())
    new_vecs = array(new_vecs)
    nearest_verts = [p_ind]*5
    orig_vecs = new_vecs.copy()
    for i, p in enumerate(vert):
        if p != near_p:
            nearest_verts.extend([i]*5)
            axis = p.cross(near_p)
            if axis.mod() == 0:
                #print axis, angle
                axis = vert[i-1].cross(near_p)
                rot_mat = axis_angle_to_matrix(axis.x, axis.y, axis.z,180)
                new_vecs = np.concatenate([new_vecs, array(np.dot(orig_vecs, rot_mat))], axis=0)
            else:
                angle = near_p.arg(p)
                rot_mat = axis_angle_to_matrix(axis.x, axis.y, axis.z, angle, rad=True)
                rot_mat = rot_mat*axis_angle_to_matrix(p.x,p.y,p.z,180)
                new_vecs = np.concatenate([new_vecs, array(np.dot(orig_vecs, rot_mat))], axis=0)
    if outfile:
        pdb = vectorlist_to_pdb([Vector.fromlist(x) for x in new_vecs])
        pdb.write_to_PDB(outfile)
    return vert, new_vecs, nearest_verts
...
def angle_for_x_axis_to_point(v, r, mat):
    to_conn = (v-r).unit()
    new_y = Vector(0,1,0).matrix_transform(mat)
    new_x = Vector(1,0,0).matrix_transform(mat)
    #conn_proj = to_conn - new_y*(to_conn.dot(new_y)/(new_y.mod()**2))
    conn_proj = to_conn - new_y*(to_conn.dot(new_y)/(new_y.dot(new_y)))
    new_xangle = new_x.arg(conn_proj)
    #print new_xangle
    cross = new_x.cross(conn_proj)
    #print new_y.dot(cross)
...
def get_icos_symm_vectors_from_run(csv, mod, vector, orient='i2', outfile=None):
    pentons, vects, nearest_penton = get_icos_symm_pos_from_vector(vector, orient=orient)
    mod_with_off = mod+csv.get_all_offsets()
    mats = csv.angles_to_rot_matrix()
    new_csv = PEETMotiveList()
    new_mod = PEETmodel()
    angles = []
    for p in range(len(mats)):
        new_points = [Vector.fromlist(point).matrix_transform(mats[p])+mod_with_off.get_vector(p) for point in vects]
        new_pentons = [point.matrix_transform(mats[p])+mod_with_off.get_vector(p) for point in pentons]
        for v in range(len(new_points)):
            new_mod.add_point(0, 0, new_points[v].to_array())
            [z1,x,z2], newmat = orient_pcle_to_point(new_points[v], mod_with_off.get_vector(p), return_matrix=True)
            new_csv.add_empty_pcle(angles=[z1,z2,x])
            ...
            r = nearest_penton[v]
            new_xangle = angle_for_x_axis_to_point(new_points[v], new_pentons[r], newmat)
            angles.append(degrees(new_xangle))
...
def get_vector_from_pos_on_average(vector, mapfile):
    emmap = MapParser.readMRC(mapfile)
    outvec = (vector-emmap.centre())/emmap.apix
    return outvec
...
def get_hexons_from_run(csv, mod, virus_diameter, tri_num=4, outfile='', orient='i2', justedges=False):
    hexons, pentons = get_hexons(virus_diameter, tri_num=tri_num, orient=orient, justedges=justedges)
    mod_with_off = mod+csv.get_all_offsets()
    mats = csv.angles_to_rot_matrix()
    new_csv = PEETMotiveList()
    new_mod = PEETmodel()
    ...
    for p in range(len(mats)):
        ...
        for v in hexons:
            ...
            current_points.append(new_point.to_array())
            new_mod.add_point(0, 0, new_point.to_array())
            [z1,x,z2], newmat = orient_pcle_to_point(new_point, mod_with_off.get_vector(p), return_matrix=True)
            new_mats.append(newmat)
            new_csv.add_empty_pcle(angles=[z1,z2,x])
            
        new_pentons = [point.matrix_transform(mats[p])+mod_with_off.get_vector(p) for point in pentons]
        new_pentons = array([po.to_array() for po in new_pentons])
        kdtree = KDTree(new_pentons)
        dists,nbrs = kdtree.query(current_points)
        for n in range(len(current_points)):
            angles.append(degrees(angle_for_x_axis_to_point(Vector.fromlist(current_points[n]),Vector.fromlist(new_pentons[nbrs[n]]), new_mats[n])))
