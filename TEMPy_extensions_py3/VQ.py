...
def VQ(D, n, epochs, alpha0=0.5, lam0=False):
	...
	# Next two lines replace the original sample_inds.append(np.nonzero(min_index)[1][0,0])
	# in order to deal with non matrix output of np.nonzero() in newer version of numpy. 
    ...
    for i in range(train_len):
        ...
        ranking[inds] = list(range(n))

        h = list(map(math.exp, -ranking/lam[i]))
